...
function setattr_mode(req, res, next) {
    ...
    if (attrs.mode !== null) {
        /*
        Not supported by BrowserFS
        fs.fs.chmod(req._filename, attrs.mode, function(err) {
            if (err) {
...
function setattr_own(req, res, next) {
    ...
    log.debug('setattr_own(%s, %d %d): entered', req._filename, attrs.uid, attrs.gid)
    ...
    getDsFs().lstat(req._filename, function(err, stats) {
        ...
        /*fs.fs.chown(req._filename, uid, gid, function(err2) {
            if (err2) {
                log.warn(err, 'setattr: chown failed')
                res.error(nfs.NFS3ERR_SERVERFAULT)
                next(false)
...
function setattr_get_mtime(req, res, next) {
    ...
    log.debug('setattr_get_mtime(%s, %d %d): entered', req._filename, attrs.how_m_time, attrs.mtime)
...
function setattr_get_atime(req, res, next) {
    ...
    log.debug('setattr_get_atime(%s, %d %d): entered', req._filename, attrs.how_a_time, attrs.atime)
...
function setattr_time(req, res, next) {
    ...
    log.debug('setattr_time(%s, %d %d): entered', req._filename, attrs.how_m_time, attrs.how_a_time)
    if (attrs.how_m_time === nfs.time_how.DONT_CHANGE && attrs.how_a_time === nfs.time_how.DONT_CHANGE) {
    ...
    /*
    Not supported by BrowserFS
    fs.fs.utimes(req._filename, req._atime, req._mtime, function(err) {
        if (err) {
