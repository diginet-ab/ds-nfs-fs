...
function access(req, res, next) {
    ...
    getDsFs().stat(req._filename, function(err, stats) {
        ...
        res.access = 0
        if (stats.mode & parseInt('0100', 8)) res.access |= nfs.ACCESS3_READ
        if (stats.mode & parseInt('010', 8)) res.access |= nfs.ACCESS3_MODIFY | nfs.ACCESS3_EXTEND | nfs.ACCESS3_DELETE
        if (stats.mode & parseInt('01', 8)) res.access |= nfs.ACCESS3_LOOKUP | nfs.ACCESS3_EXECUTE
...
export default function chain() {
    return [common.fhandle_to_filename, access]
}
